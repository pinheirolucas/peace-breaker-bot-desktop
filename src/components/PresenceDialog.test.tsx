...
import { usePanelShortcut } from "../hooks/usePanelShortcut";
...
describe("PresenceDialog", () => {
  ...
  it("persists nothing until Pronto", async () => {
    ...
    expect(onConfirm).toHaveBeenCalledWith(
      expect.objectContaining({ tray: true, panel: true }),
  ...
  it("cannot confirm quick access without the icon", async () => {
    ...
    expect(onConfirm.mock.calls[0][0]).toMatchObject({ tray: false, panel: false });
  });
...
describe("usePanelShortcut", () => {
  ...
  function bridge(result: { registered: string[]; failed: { key: string; reason: string }[] }) {
    ...
    window.instantsPanel = { action: vi.fn(), setShortcut: setShortcut as never, onShown: () => () => {} };
    return setShortcut;
  }

  it("registers only while it and the panel are on", async () => {
    ...
    const { rerender } = renderHook(({ panel }) => usePanelShortcut(panel, vi.fn()), { initialProps: { panel: true } });
  ...
  it("snaps the switch back and says so when another app holds the combination", async () => {
    ...
    renderHook(() => usePanelShortcut(true, onTaken));

    await waitFor(() => expect(onTaken).toHaveBeenCalledTimes(1));
    expect(JSON.parse(window.localStorage.getItem("panelShortcut") ?? "{}").enabled).toBe(false);
  });
