...
## Configurações (settings window)

A second, ordinary `BrowserWindow` (`electron/settingsWindow.ts`): one instance, not modal, 880x620 (min 640x460), bounds remembered in `userData/settings-window.json`, same preload and sandbox settings. It is the same `index.html` at `#/settings[/section]`; `index.tsx` branches on the hash (`isSettingsHash`), so Vite and tsup are unchanged. `electron/settings.ts` is pure and inlined into the preload: the seven section ids (general, appearance, server, explore, keys, presence, data), the channels, `isSettingsOpenRequest`, `restoreBounds`. Main validates every request (fixed section list; sender must be the main or settings window). `chrome:set` is accepted from both windows and paints the *sender's* own window.

**The settings window only WRITES stored values.** It must never mount `useGlobalShortcuts`, `useQuickAccessShortcut`, `useReportPresenceSettings` or anything else that pushes settings to main: only the main window does, and main refuses shortcut/presence requests from the settings window (`fromAppWindow` excludes it). The storage event (`src/lib/persisted.ts`) carries a change to the main window. Aparência is a launcher, not a pane: it sends `settings:open-appearance`; main raises the main window and sends the existing `appearance` menu command (`useAppearance().begin()`); the stage, dock and `useAppearance` are untouched. Everything applies at once (no Salvar). `src/settings/*Section.tsx` each export a presentational `*Pane` (used by Storybook and tests) and a container wired to the hooks. Restaurar configurações (`resetSettings`, `settingsKeys` in `src/storage.ts`) clears settings keys through `clearPersisted` and never `instants`. Doors: Cmd/Ctrl+, (native item, main opens it directly), the overflow menu, the tray menu, ServerMenu's and FilterMenu's footers, and Reprodução › Atalhos globais….

## Command palette (Cmd/Ctrl+K)

`src/components/CommandPalette.tsx` is a Radix modal opened from any tab (search field included) by Cmd/Ctrl+K in `App.tsx`, and shut while another dialog, a menu or the Aparência shell is open; the shortcut again closes it. The rules (rows, ranking, groups, the `>` actions-only mode) are pure in `src/lib/palette.ts`; `App.tsx` builds the rows, each carrying the handler its own button, menu or card already runs (`paletteCandidates`). It closes *before* it runs a row. Sounds go through `FavoritesPanel`'s `controlsRef` (`FavoritesControls.play/stop`), i.e. `triggerInstant` and `cardState`, so a sound the card refuses (bot known away, another clip playing, Organizar) is refused here too; `null` bot status is never "away". Recentes are the last three played (`recentClips` key, not a setting). The palette pick and colour mode preview through the **one** `useAppearance` draft: `beginPreview()` makes a draft with no shell (`previewing`, as against `editing`, which is the Aparência stage), each highlighted row calls `preview`, Enter calls `commit(patch)`, and every other way out (Esc, click outside, Cmd/Ctrl+K) is `cancel()` — the palette stays mounted while closed so a closing palette can always drop its preview. Rows are rebuilt on every render, so the preview effect is keyed on the payload, not the row: keying on the row loops.

## Favourite keys

A favourite can have one key, `Instant.key` (`a`–`z` or `0`–`9`, absent means none, so old lists and backups load unchanged). `src/lib/clipKeys.ts` holds the pure rules: `clipKeyFromEvent` (matched on `event.key`, with `event.code` only for Shift + a digit), `assignKey` (one key per sound and one sound per key, returning the sound it displaced for the undo toast), and the import rules (`mergeImported` drops an imported key that collides with a stored one, `sanitizeKeys` de-duplicates a replacing file).

`useClipShortcuts` is the window listener, in `FavoritesPanel` because that owns both players. Bare key plays on Discord, Shift + key plays here, Esc stops. Every guard is in the hook: never in an editable target, behind an open dialog or menu (Aparência's dock is one), in Organizar, or with Cmd/Ctrl/Alt held. `triggerClip(key, mode)` in `FavoritesPanel` asks `cardState` what a click would be allowed to do, so a key can never do what the card's own buttons refuse; it is also the path the global keys take. Because keys must work from Explorar, `App.tsx` gives Favoritos' `Tabs.Content` `forceMount` and hides it itself (Radix does not); the panel takes an `active` prop so a hidden one never writes its summary over Explorar's. `?` (App) opens `ShortcutSheet`; keys are set in Organizar through `ShortcutDialog`.

### Global keys

Off by default, switched on in the sheet. The same key plus one modifier reaches the app while another has focus. `electron/shortcuts.ts` is pure and inlined into the preload like `chrome.ts`: the three modifier presets per OS (`modifiersFor`; Ctrl+Alt is not the default off macOS because it is AltGr on ABNT2), `acceleratorFor`, and `isShortcutRequest`, which the main process runs on every request from the untrusted renderer. Main knows keys, never urls. It unregisters only what it registered, debounces a repeat within 250 ms, and releases everything on `will-quit` and on the window closing. It also takes the single-instance lock, and on macOS while global keys are on the close button hides the window instead (the renderer is what plays, so a global key works exactly as long as the window exists). `useGlobalShortcuts` sends a request only when the enabled/modifier/sorted-keys string changes. Under Wayland `main.ts` enables Chromium's `GlobalShortcutsPortal` feature and reports a failure as `"unsupported"` rather than `"in-use"`: that flag is not verified against the pinned Electron.
...
in Configurações › Geral (Automático / Português / English; Automático removes the `language` key).
